...
model <- lmer(CHI_MLU~VISIT* Diagnosis+ I(VISIT^2)*Diagnosis +I(VISIT^3)* Diagnosis +(1|SUBJ), full_data, REML = F)
summary(model)
...
fixef(model)["DiagnosisB:I(VISIT^2)"] <- 0.1
fixef(model)["DiagnosisB:I(VISIT^3)"] <- 0.1


powerCurveV = powerCurve(model,fixed("VISIT", method = "t"), along =  "SUBJ", nsim=200)
 plot(powerCurveV)
...
powerCurveI3 = powerCurve(model, fixed("DiagnosisB:I(VISIT^3)", method = "t"),along="SUBJ", nsim=200)
plot(powerCurveI3)
...
#adding participants
model_ext_class <- extend(model, along="SUBJ", n=100)
...
powerCurveI = powerCurve(model_ext_class,fixed("VISIT", method = "t"),along="SUBJ", nsim=200)
powerCurveI
