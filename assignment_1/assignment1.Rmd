...
demo__train <- read.csv("demo_train.csv")
...
#we need to see which variebles are interesting ffor us according to the experiment was abput, number of words, uniques words, morpheme in respect to time
#token - total amount of words
#types- uniques words

hist(token_train$tokens_MOT)
hist(token_train$tokens_CHI) #change in time, amount of children with disoders
hist(token_train$types_MOT)

hist(token_train$types_CHI)

summary(demo__train)
#change names

token_train <- rename(token_train, Child.ID=SUBJ)
LU_train <- rename(LU_train, Child.ID=SUBJ)

token_train <- rename(token_train, visit=VISIT)
LU_train <- rename(LU_train, visit=VISIT)
demo__train <- rename(demo__train, visit=Visit)
```
...
```{r}
#we are looking inside dateset to see how many different patterns we have within one column
#\\d - means digits
#when we aasign to dataset we also change dataset to have just digits instead of words and digits.
str_extract(demo__train$visit, "\\d+")
...
```{r}
LU_train$Child.ID <- str_replace_all(LU_train$Child.ID, "\\.", "")
demo__train$Child.ID <- str_replace_all(demo__train$Child.ID, "\\.", "")
token_train$Child.ID <- str_replace_all(token_train$Child.ID, "\\.", "")

combined_data <- demo__train %>% full_join(LU_train, by = "Child.ID")
combined_data <- combined_data %>% full_join(token_train, by = "Child.ID")
...
```{r}
#creating new data frame, where we only have variables we need
selected_data <- select(combined_data, Child.ID, visit, Ethnicity, Diagnosis, Gender, Age, ADOS,  MullenRaw, ExpressiveLangRaw, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd, types_MOT, types_CHI, tokens_MOT, tokens_CHI)

#renaming variables as advised
selected_data <- rename(selected_data, nonverbalIQ="MullenRaw")
selected_data <- rename(selected_data, verbalIQ="ExpressiveLangRaw")
...
```{r}
#selecting  only 1st visit
first_visit <- filter(selected_data, Visit==1)
#renaming collums
first_visit <- rename(first_visit, ADOS1="ADOS", nonverbalIQ1="nonverbalIQ", verbalIQ1="verbalIQ")

#combining data ???
total_data <- first_visit%>% right_join(selected_data)
...
#arrange 
#data %>% filter(visit==6) %>% arrange(desc(tokens_CHI)) %>% head() - select 1st 6x rows
#slice(1)
