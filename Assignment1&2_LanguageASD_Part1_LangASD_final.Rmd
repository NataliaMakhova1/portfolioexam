...
setwd("C:/Users/natal/Google Drive/study/3 semester/EM3/portfolioexam/assignment_1")
...
merged_data2 <- dplyr::rename(merged_data2, ADOS = ADOS1)
merged_data2 <- dplyr::rename(merged_data2, verbalIQ = verbalIQ1)
merged_data2 <- dplyr::rename(merged_data2, nonverbalIQ = nonverbalIQ1)
...
LU3 <- LU_data1 %>% 
  ...
  filter(SUBJ) %>% filter(VISIT == c( "Visit1.", "Visit2.", "Visit3.", "Visit4.", "Visit5.", "Visit6."))
...
final_data %>% filter(VISIT == 6) %>%  arrange(desc(tokens_CHI)) %>%  slice(1)
...
#we can't talk about development over time, when we have just one average number for tokens and mean lenght of utterance.
```
